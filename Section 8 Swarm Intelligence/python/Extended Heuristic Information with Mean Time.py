...
def compute_heuristic(mean_cost, variance_cost, mean_time, lambda_factor):
    heuristic = np.divide(1, (mean_cost + lambda_factor * (variance_cost + mean_time)), out=np.zeros_like(mean_cost), where=(mean_cost + lambda_factor * (variance_cost + mean_time))!=0)
    heuristic[mean_cost == np.inf] = 0
    return heuristic

heuristic = compute_heuristic(mean_cost, variance_cost, mean_time, lambda_factor)
