...
def evaluate(position):
    # Ensure the position is treated as a 2D array for differential calculation
...
try:
    ...
    logging.info(f"Optimization complete. Best Score: {gbest_score}, Best Position: {gbest_position}")

except Exception as e:
    logging.error(f"Error during PSO execution: {e}")
    raise

# Detailed Reporting
print(f"Positions Shape: {positions.shape}")
print(f"Velocities Shape: {velocities.shape}")
print(f"pbest_positions Shape: {pbest_positions.shape}")
print(f"gbest_position Shape: {gbest_position.shape}")
