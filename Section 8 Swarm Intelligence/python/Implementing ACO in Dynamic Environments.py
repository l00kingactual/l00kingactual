...
class AntColonyOptimizerVRP:
    ...
    def construct_solution(self, distance_matrix, pheromones):
        num_cities = len(distance_matrix)
        solution = [[self.depot] for _ in range(self.num_vehicles)]
        remaining_cities = set(range(num_cities)) - {self.depot}
...
def dynamic_distance_matrix(num_cities):
...
np.random.seed(0)
num_cities = 20
distance_matrix = dynamic_distance_matrix(num_cities)

aco_vrp = AntColonyOptimizerVRP(num_ants=10, num_iterations=50, alpha=1, beta=5, rho=0.1, pheromone_deposit=1, num_vehicles=3, depot=0)

for iteration in range(5):  # Simulate dynamic environment over multiple iterations
    distance_matrix = dynamic_distance_matrix(num_cities)
    ...
    time.sleep(2)  # Wait before next iteration to simulate real-time changes
