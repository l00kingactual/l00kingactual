...
ALIGNMENT_RADIUS = 50  # Radius within which boids try to align with neighbors
COHESION_RADIUS = 50  # Radius within which boids try to move towards the center of mass of neighbors
...
class Boid:
    ...
    def update(self, boids):
        ...
        # Calculate forces based on neighboring boids
        for other in boids:
            if other is self:
                continue  # Skip self
            ...
            if distance < SEPARATION_RADIUS:
                separation += (self.position - other.position) / distance  # Add repulsive force to separation force
                count_separation += 1
