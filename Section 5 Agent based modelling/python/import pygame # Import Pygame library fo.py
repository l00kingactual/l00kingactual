import pygame  # Import Pygame library for graphical simulation
import random  # Import Random library for generating random positions and velocities
...
class Food:
    ...
    def draw(self, screen):
        ...
        pygame.draw.circle(screen, FOOD_COLOR, self.position.astype(int), 10)  # Draw the food source
...
def main():
    ...
    food_sources = [
        Food(random.randint(50, WIDTH - 50), random.randint(50, HEIGHT - 50)) for _ in range(NUM_FOOD_SOURCES)
    ]  # Random food sources spread out
