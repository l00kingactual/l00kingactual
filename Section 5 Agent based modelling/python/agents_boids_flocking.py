...
class Boid:
    ...
    def update(self, boids):
        ...
        if neighbor_count > 0:
            ...
            self.vx += (align_vx - self.vx) * 0.05
            self.vy += (align_vy - self.vy) * 0.05

            # Steer towards the center of mass (cohesion)
            self.vx += (cohesion_x - self.x) * 0.01
            self.vy += (cohesion_y - self.y) * 0.01

            # Steer to avoid neighbors (separation)
            self.vx += separation_vx * 0.1
            self.vy += separation_vy * 0.1
