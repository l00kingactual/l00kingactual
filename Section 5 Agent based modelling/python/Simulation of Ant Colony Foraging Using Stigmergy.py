...
NUM_FOOD_SOURCES = 4  # Number of food sources
VISUAL_RANGE = 50  # Distance at which ants can see food or pheromone trails
...
def main():
    ...
    while running:
        ...
        for pheromone in pheromones:
            pheromone.evaporate()  # Evaporate pheromones over time
            if pheromone.strength > 0:
                alpha = min(255, pheromone.strength * 255 / PHEROMONE_STRENGTH)  # Calculate alpha for transparency
                color = (*PHEROMONE_COLOR[:3], alpha)
                pygame.draw.circle(screen, color, pheromone.position.astype(int), ANT_SIZE)
        ...
        for ant in ants:
            ant.update(pheromones)  # Update each ant's position and state
