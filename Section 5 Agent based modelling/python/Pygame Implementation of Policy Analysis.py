...
class Seller:
    def __init__(self):
        ...
        self.base_price = random.uniform(MIN_PRICE, MAX_PRICE - CARBON_TAX)
        self.price = self.base_price + CARBON_TAX
    ...
    def update(self, buyers):
        ...
        if demand > self.goods:
            self.base_price = min(MAX_PRICE - CARBON_TAX, self.base_price + PRICE_ADJUSTMENT)
        elif demand < self.goods:
            self.base_price = max(MIN_PRICE, self.base_price - PRICE_ADJUSTMENT)
        self.price = self.base_price + CARBON_TAX
...
pygame.display.set_caption("Policy Analysis Simulation")
