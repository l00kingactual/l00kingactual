...
def gravity(pos_i, pos_j, mass_i, mass_j, G=0.1):
    r = np.linalg.norm(pos_j - pos_i)
    if r == 0:
        return np.zeros(2)
    force_magnitude = G * (mass_i * mass_j) / r**2
    force_direction = (pos_j - pos_i) / r
        ...
        try:
            ...
            for neighbor in neighbors:
                if not np.any(np.isnan(neighbor.position)) and not np.any(np.isnan(neighbor.velocity)):
                    ...
                    gravity_force += function_dict["gravity"](self.position, neighbor.position, self.mass, neighbor.mass)

            total_force = sep_force + coh_force + alignment_force + gravity_force
...
while running:
    ...
    for agent in agents:
        neighbors = [other for other in agents if np.linalg.norm(agent.position - other.position) < 200 and other != agent]
