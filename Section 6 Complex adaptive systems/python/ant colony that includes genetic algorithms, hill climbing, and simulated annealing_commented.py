...
NEST_COLOR = (128, 0, 128)  # Purple color for nest
FOOD_COLOR = (0, 255, 0)  # Green color for food
THREAT_COLOR = (255, 0, 0)  # Red color for threats
NEST_RESOURCE_COLOR = (0, 0, 255)  # Blue color for nest resources
ANT_COLOR = {
    'forager': (255, 255, 255),  # White color for forager ants
    'builder': (0, 0, 255),  # Blue color for builder ants
    'defender': (255, 255, 0)  # Yellow color for defender ants
}
...
NUM_THREATS = 10
INTERACTIONS_REQUIRED = 20  # Number of interactions required to deplete a resource or threat
...
class Ant:
    def __init__(self, x, y, role):
        ...
        self.carrying_food = False  # Whether the ant is carrying food (only relevant for foragers)
            ...
            if self.role == 'forager':
                if self.carrying_food:
                    ...
                    if np.linalg.norm([self.x - nest[0], self.y - nest[1]]) < 5:
                        self.carrying_food = False  # Dropped off food at nest
                else:
    ...
    def draw(self, screen):
        pygame.draw.circle(screen, ANT_COLOR[self.role], (int(self.x), int(self.y)), 2)  # Draw ant on the screen
