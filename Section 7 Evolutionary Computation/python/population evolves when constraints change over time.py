...
def evaluate(individual):
    ...
    f2 = np.prod(individual)
    return f1, f2

# Dynamic constraint function
def constraint(individual, gen):
    return np.sum(individual) - (5 + 5 * np.cos(gen / 20.0))  # Dynamic constraint

# Penalty function
def penalty(individual, gen):
    constraint_violation = max(0, constraint(individual, gen))  # Constraint violation if positive
    penalty_factor = 100  # Less severe penalty factor
...
def plot_evolution(fitness_data):
    ...
    palette = sns.color_palette("coolwarm", NGEN)
    ...
    for gen, fitness_gen in enumerate(fitness_data):
        ...
        plt.scatter(f1, f2, color=palette[gen], label=f'Generation {gen}' if gen % 10 == 0 else "", alpha=0.6, edgecolors='w', s=40)
    
    plt.xlabel('Objective 1')
    plt.ylabel('Objective 2')
    plt.title('Evolution of Population Over Generations with Dynamic Constraint')
