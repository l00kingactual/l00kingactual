...
def initialize_population(size):
    return [IndividualImage() for _ in range(size)]

def calculate_fitness_parallel(individual, target_image):
...
def genetic_algorithm_image(target_image):
    try:
        ...
        for generation in range(GENERATIONS):
            ...
            population = pool.starmap(calculate_fitness_parallel, [(ind, target_image) for ind in population])
            ...
            while len(next_generation) < population_size:
                ...
                child = IndividualImage.crossover(parents[0], parents[1])
