...
def evolve_es(population_size, generations, dimension):
    ...
    best_individual = population[np.argmin([sphere(ind) for ind in population])]
...
try:
    ...
    print(f"Best solution: x = {best_solution}, f(x) = {sphere(best_solution)}")
    ...
    plt.ylabel('Fitness')
    plt.title('Evolution Strategies Optimization of Sphere Function')
    plt.legend()
    plt.show()
