...
# Objective function for a complex optimization problem (high-dimensional)
def objective_function(x):
    try:
        # A more complex function to simulate an NP-Hard optimization problem
        result = np.sum(np.sin(x)**2 + np.cos(x)**2 + x**4)  # Example function with multiple local minima
        return result
