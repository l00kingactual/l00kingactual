...
def simulate_transitions(n):
    ...
    fig.add_trace(go.Scatter(x=list(range(len(gray_code_sequence))), y=gray_code_sequence, mode='lines+markers', name='Gray Code Sequence'), row=1, col=1)
    fig.add_trace(go.Scatter(x=list(range(len(decimal_sequence))), y=decimal_sequence, mode='lines+markers', name='Decimal Sequence'), row=2, col=1)
