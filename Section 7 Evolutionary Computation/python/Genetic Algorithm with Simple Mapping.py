...
mutation_rate = 0.01
crossover_rate = 0.7
dimension = 5  # Number of dimensions for the Sphere function
...
def evaluate_population(population):
    ...
    return np.array([sphere_function(pheno) for pheno in phenotypes])
...
def select_parents(population, fitness):
    ...
    probabilities = 1 / fitness  # Convert to maximization problem
...
print(f"Best solution: x = {best_solution}, f(x) = {sphere_function(best_solution)}")
...
plt.ylabel('Fitness')
plt.title('Genetic Algorithm Optimization of Sphere Function')
plt.legend()
plt.show()
