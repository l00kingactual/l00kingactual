...
def save_ai_mi_output(cube, filename):
    ai_mi_dict = cube.ai_mi_dictionary()
    with open(filename, 'w') as file:
        for principle, tactics in ai_mi_dict.items():
            file.write(f"Strategic Principle: {principle}\n")
            for general, ideas in tactics.items():
                file.write(f"  {general}: {', '.join(ideas)}\n")
            file.write("\n")

# Assuming StrategyCube class is correctly defined and initialized
cube = StrategyCube()
save_ai_mi_output(cube, 'strategy_cube_output.txt')